  ...
  case $1 in
    windowtitle*)
      ...
      if [[ "$window_title" == *"(Bitwarden Password Manager) - Bitwarden"* ]]; then
        ...
        hyprctl --batch "dispatch togglefloating address:0x$window_id ; dispatch resizewindowpixel exact 30% 50%,address:0x$window_id ; dispatch movewindowpixel exact 40% 30%,address:0x$window_id"        
      fi
      ;;
  esac
}
